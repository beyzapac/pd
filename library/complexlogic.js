...
function unitcircCal(){
    ...
    if(deg==""||isNaN(deg))
    {
        document.getElementById("unitcircxans").innerHTML = "Lütfen boş bırakmayın";
...
function wmccal()
{
    ...
    if(num1==""||num2=="")
    {
        ans="Tüm değerleri eksiksiz girin";
    }
    else{
    ...
    if(len1!=len2)
    {
        ans="Veri sayınız ve ağırlığınız eşit değil ";
    }
    else
    {
        var ans1=[];
...
function perchngCal(){
    ...
    if (!isNaN(initial) && !isNaN(final))
    {
        ...
        temp += "\\[Yüzdelik \\space değişim \\space şu \\space şekilde \\space olur,\\]"
        temp += "\\[\\space = \\space \\frac{((Son Değer) - (Başlangıç Değeri))}{ |(Başlangıç Değeri)| } \\times 100\\]"
        ...
        temp += "\\[\\space = \\space"+ans.toFixed(2)+" \\space \\% lik \\space değişim \\space var \\space \\]"
...
{ 
    ...
    {
        if(b<0){
            temp += "\\[Z \\space Score = \\frac{Ham \\space Değer\\ (X) \\space - \\space Popülasyon\\space Ortalaması \\space (μ)}{Standard \\space Sapma \\space (σ)}\\]"
            ...
            renderMathInElement(ans);
              ...
              temp += "\\[Z \\space Score \\space = \\space \\frac{Ham \\space Değer \\space (X) \\space - \\space Popülasyon \\space Ortalaması \\space (μ)}{Standard \\space Sapma \\space (σ)} \\]"
              temp += "\\[Z \\space Score \\space = \\space \\frac {" + a + " - " + b + "}{" + c + "} \\space = \\space \\frac{" + (a-b) + "}{" + c + "} \\]";
              temp += "\\[Z \\space Score \\space = \\space " + z.toFixed(4) + " \\]"
              ans.innerHTML = temp;
              renderMathInElement(ans);
...
function clockcal()
{
    ...
    if(a==""||b=="")
    {
        ans="Lütfen boş değer bırakmayın";
    }
    else
    {
        a=parseFloat(a);
        ...
        ans="Dakikadan saat ibresine açı: "+Math.abs(anghour-angmin)+" derece";
        ans+="<br>"
        ans+="Saatten dakika ibresine açı: "+Math.abs(360-Math.abs(anghour-angmin))+" derece";
    }
